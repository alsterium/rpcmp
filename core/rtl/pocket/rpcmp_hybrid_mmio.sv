...
module rpcmp_hybrid_mmio (
    ...
    logic clear_request, start_request, pause_request, repeat_request, started;
    (* preserve, altera_attribute = "-name SYNCHRONIZER_IDENTIFICATION FORCED_IF_ASYNCHRONOUS" *)
    logic [1:0] clear_sync, start_sync, pause_sync, repeat_sync, ack_sync;
    (* preserve, altera_attribute = "-name SYNCHRONIZER_IDENTIFICATION FORCED_IF_ASYNCHRONOUS" *)
    logic [7:0] status_meta, status_sync;
    logic clear_ack;
    logic [12:0] reset_cycles;
    logic [4:0] release_cycles;
    wire audio_clear=clear_sync[1];
    wire running, ended, paused, repeat_ack;
    wire [3:0] faults;
    always_ff @(posedge clk_audio or negedge audio_reset_n) begin
        if (!audio_reset_n) begin
            clear_sync<=3; start_sync<=0; pause_sync<=0; repeat_sync<=0; clear_ack<=0; reset_cycles<=0; release_cycles<=0;
        end else begin
            clear_sync<={clear_sync[0],clear_request}; start_sync<={start_sync[0],start_request};
            pause_sync<={pause_sync[0],pause_request};
            repeat_sync<={repeat_sync[0],repeat_request};
    ...
    always_ff @(posedge clk_cpu or negedge cpu_reset_n) begin
        if (!cpu_reset_n) begin ack_sync<=0; status_meta<=0; status_sync<=0; end
            ...
            status_meta<={repeat_ack,paused,faults,ended,running}; status_sync<=status_meta;
        end
    end
    ...
    wire [63:0] pcm_head;
    wire [48:0] fm_head;
    // An autonomous fade can end during an already-admitted CPU block write.
    // Its bounded FIFO commits remain valid; Clear discards the unused tail.
    wire available=control_state == READY && status_sync[5:2] == 0;
    ...
    rpcmp_hybrid_audio audio (
        ...
        .repeat_request(repeat_sync[1]), .repeat_ack(repeat_ack),
    ...
    always_comb begin
        ...
        if (read && write) error=1;
            ...
            if (address[9:8] != 0 || address[1:0] != 0 || byte_enable != 4'hf || !cpu_reset_n) error=1;
                ...
                8'h00: read_data=32'h48594234;
                8'h04: read_data={22'd0,status_sync[7:6],status_sync[5:2],status_sync[1],status_sync[0],started,
                                  control_state == READY};
            ...
            endcase
                ...
                8'h08: begin
                    ...
                    else if (write_data == 16)
                        error=!available || repeat_request != status_sync[7];
                    else error=1;
                end
    ...
    always_ff @(posedge clk_cpu or negedge cpu_reset_n) begin
        if (!cpu_reset_n) begin
            control_state<=CLEAR_WAIT; clear_request<=1; start_request<=0; pause_request<=0; repeat_request<=0; started<=0;
            ...
            if (write && !error) case (address)
                8'h08: if (write_data == 1) begin
                    clear_request<=1; start_request<=0; pause_request<=0; repeat_request<=0; started<=0; control_state<=CLEAR_WAIT;
                    left_valid<=0; at_valid<=0; last_at<=0; end_queued<=0;
                end else if (write_data == 2) begin start_request<=1; started<=1; end
                else if (write_data == 16) repeat_request<=~repeat_request;
