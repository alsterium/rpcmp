module rpcmp_spike_regs #(
    ...
    always_comb begin
        ...
        if (bridge_rd && (bridge_addr == STATUS_ADDR)) begin
            ...
            bridge_rd_data = injected_tick[63:32];
        end else if (bridge_rd && (bridge_addr == BUILD_SIGNATURE_ADDR)) begin
            bridge_rd_data = BUILD_SIGNATURE;
        end else if (bridge_rd && (bridge_addr == WRITE_SEQUENCE_ADDR)) begin
            bridge_rd_data = write_sequence;
        end else if (bridge_rd && (bridge_addr == LAST_WRITE_ADDR_ADDR)) begin
            bridge_rd_data = last_write_addr;
        end else if (bridge_rd && (bridge_addr == LAST_WRITE_DATA_ADDR)) begin
