...
module rpcmp_native_completion #(parameter integer PAYLOAD_WIDTH = 1) (
    ...
    input logic receipt_update,
    input logic [PAYLOAD_WIDTH-1:0] receipt_payload,
    output logic [PAYLOAD_WIDTH-1:0] native_payload,
    output logic [63:0] native_prefix,
    output logic completion_fault
);
    localparam logic [64:0] DELAY_EDGES = 65'd5273;
    logic [128+PAYLOAD_WIDTH:0] entries[0:31];
    logic [4:0] read_index, write_index;
    logic [5:0] count;
    logic [128+PAYLOAD_WIDTH:0] head;
    ...
    always_ff @(posedge clk_audio) begin
        if (store_entry) entries[write_index] <= {receipt_update,receipt_payload,receipt_token,due_sum[63:0]};
    ...
    always_ff @(posedge clk_audio or negedge reset_n) begin
        if (!reset_n) begin
            ...
            native_prefix<=0; native_payload<='0; completion_fault<=0;
            ...
            if (retire) begin
                ...
                if (head[128+PAYLOAD_WIDTH]) native_payload<=head[128 +: PAYLOAD_WIDTH];
