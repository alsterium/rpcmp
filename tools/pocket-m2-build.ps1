...
foreach($name in @('rpcmp_device_queue.sv','rpcmp_pocket_audio.sv','rpcmp_jt51_audio.sv',$coreSource)){
...
$top=$top.Substring(0,$start)+"// RPCMP $($Variant.ToUpperInvariant()) audio is instantiated after the audio PLL.`n`n"+$top.Substring($end)
$pllMarker=@'
mf_pllbase mp1 (
    .refclk         ( clk_74a ),
    .rst            ( 0 ),
...
'@.Replace("`r`n","`n").Replace('$coreModule',$coreModule)
...
$sdc=@'
# RPCMP audio clock-domain and external-interface constraints.
...
$evidence=[ordered]@{
    Variant=$Variant;Quartus='25.1std.0 Build 1129';TemplateRevision=$templateRevision;JT51Revision=$jt51Revision
...
Write-Output "Pocket $($Variant.ToUpperInvariant()) Quartus validation: PASS"
