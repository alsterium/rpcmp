...
try {
    ...
    $lines = @(& $vsim -c -quiet -lib work jt51_audio_tb -do 'onerror {quit -code 1}; run -all; quit -code 0' 2>&1)
    $code = $LASTEXITCODE; $lines | Write-Output; $text = $lines | Out-String
    if ($code -ne 0 -or $text -notmatch '(?m)^# jt51_audio_tb: PASS writes=30\r?$' -or
        $text -notmatch '(?m)^# Errors: 0, Warnings: 0\r?$') { throw 'Real JT51 integration simulation failed.' }
