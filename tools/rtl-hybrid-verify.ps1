[CmdletBinding()]
param([string]$Jt51 = 'out/research/jt51-985a573', [int[]]$PhasePs = @(0, 5555, 81379), [switch]$VoiceOnly)
...
$sources += @('hybrid_mixer_tb','hybrid_stream_tb') |
    ForEach-Object { Join-Path $repo "tests/rtl/$_.sv" }
...
try {
    ...
    foreach ($run in $runs) {
        ...
        if ($code -ne 0 -or !$log.Contains($run.Marker) -or
