...
module m2_fixed_core_tb;
    ...
    rpcmp_m2_fixed_core #(.CPU_TICKS_PER_SECOND(10000)) dut (.*);
    ...
    initial begin
        ...
        if(writes!=30 || max_samples==0) $fatal(1,"sequence evidence writes=%0d samples=%0d",writes,max_samples);
        if(queue_fault || audio_fault) $fatal(1,"fault queue=%0d audio=%0d",queue_fault,audio_fault);
        if(observed[0]!==16'h0020 || observed[1]!==16'h01c7 ||
           observed[28]!==16'h0008 || observed[29]!==16'h0100)
            $fatal(1,"ordered endpoints");
        $display("m2_fixed_core_tb: PASS writes=%0d samples=%0d",writes,max_samples);$finish;
    end
endmodule
